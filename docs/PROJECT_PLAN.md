...
- No forced Chrome Profile isolation.
...
1) Workspace file generator:
   - Writes `~/.config/project-workspaces/vscode/<projectId>.code-workspace`
   - Includes:
...
1) Chrome launch behavior:
   ...
   - Tabs are opened by launching Chrome with `--new-window` and URLs.
